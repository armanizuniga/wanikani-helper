// A read-only snapshot of one burned subject's local practice record, used by the Burned Stats
// screen. A value type rather than the SwiftData model so the view can sort, partition and rank
// freely, and so one type covers both the kanji and the vocabulary schedule — the ranking rules
// below are the same for each, and are testable on their own.
import Foundation

struct BurnedStat: Identifiable, Equatable {
    let subjectId: Int
    let stage: Int                  // 0–7, indexes BurnedSRS.intervalDays
    let dueAt: Date
    let burnedAt: Date?
    ...
    /// False for a subject WaniKani has burned that this app has never quizzed — it has no record
    ...
    /// single bad day would hold an item in "Needs Work" forever no matter how well it was known
    ...
    /// True when the item currently reads as solid: never missed here, or missed and since
    ...
    /// True for an item that was missed here and has since earned its way back.
    ...
    /// True accuracy, for display. Nil until the item has actually been practiced here.
    var accuracy: Double? {
    ...
    /// score 0, so the lists would be dominated by whichever item happened to be answered once.
    ...
    /// outranks a single lucky answer in "Strongest". Which list an item lands in is decided by
